...
# make the CSV
# read PMA times
with open('outputs/pma_map_range_scaling.out', newline='') as csvfile:
...
# read CPMA times
with open('outputs/cpma_map_range_scaling.out', newline='') as csvfile:
    reader = csv.reader(csvfile, delimiter=' ')
    idx = 0
    for row in reader:
...
exp_finds_per_search = (num_elts * range_size) / universe_size
exp_finds_total = exp_finds_per_search * num_queries

for i in range(0, num_entries):
    pma_scaling[i] = pma_times[0] / pma_times[i]
    cpma_scaling[i] = cpma_times[0] / cpma_times[i]
    pma_throughput[i] = exp_finds_total / pma_times[i]
    cpma_throughput[i] = exp_finds_total / cpma_times[i]
...
plt.savefig("./plots/map_range_scaling.pdf", format="pdf")

# make the CSV

with open('./csvs/map_range_scaling.csv', 'w', newline='') as file:
